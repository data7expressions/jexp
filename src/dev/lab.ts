...
(async () => {
  const list = [
    ...
    { cmd: 'curl -s https://raw.githubusercontent.com/FlavioLionelRita/jexp/main/data/orders.json | jexp \'.details.sum(p=> p.unitPrice * p.qty )\'', desc: 'Get the sum "unitPrice * p.qty" of the details of item 1 of the list using pipeline' },
		...
		{ cmd: 'jexp \'.details.last(p=> p.unitPrice * p.qty < 3 ).article', desc: 'Get the article of the last detail where the subtotal is less than 3' },
		...
		{ cmd: 'jexp \'.details.distinct(p=>p.article)\' ./data/orders.json', desc: 'Get the list of items without repeating' },
		{ cmd: 'jexp \'.details.distinct(p=>{article:p.article,qty:p.qty})\' ./data/orders.json', desc: 'Get the list of items and quantity without repeating' },
		{ cmd: 'jexp \'.details.map(p=>{article:p.article,count:count(1),total:sum(p.qty * p.unitPrice)})\' ./data/orders.json', desc: '' },
		{ cmd: 'jexp \'{total:.[0].details.sum(p=>p.qty * p.unitPrice)}\' ./data/orders.json', desc: 'Get the total of the first order' },
    { cmd: 'jexp \'{total:.[.length()-1].details.sum(p=>p.qty * p.unitPrice)}\' ./data/orders.json', desc: 'Get the total of the last order' },
